...
//修改第三方保险
processor.call('updateAnSaleOrder', (db: PGClient, cache: RedisClient, done: DoneFunction, domain: any, order_id: string, items: any, callback: string) => {
...
processor.call("createUnderwrite", (db: PGClient, cache: RedisClient, done: DoneFunction, uwid: string, oid: string, plan_time: any, validate_place: string, validate_update_time: any, callback: string) => {
  ...
  let pcreate = new Promise<void>((resolve, reject) => {
    db.query("INSERT INTO underwrites (id, oid, plan_time, validate_place, validate_update_time) VALUES ($1, $2, $3, $4, $5)", [oid, plan_time, validate_place, validate_update_time], (err: Error) => {
      if (err) {
        log.info("err" + err);
        reject(err);
      } else {
...
// 提交审核结果
processor.call("submitUnderwriteResult", (db: PGClient, cache: RedisClient, done: DoneFunction, uwid: string, underwrite_result: string, update_time: any, callback: string) => {
  log.info("submitUnderwriteResult ");
  modifyUnderwrite(db, cache, done, uwid, callback, "UPDATE underwrites SET underwrite_result = $1, result_update_time = $2, updated_at = $3 WHERE id = $4", [underwrite_result, update_time, update_time, uwid], (underwrite) => {
    underwrite["underwrite_result"] = underwrite_result;
    underwrite["result_update_time"] = update_time;
    underwrite["updated_at"] = update_time;
    log.info("underwirte--------" + underwrite);
    return underwrite;
  });
});
